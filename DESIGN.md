...
## 🛠️ Priority & Developer Guidelines

Ketika bekerja pada aplikasi mobile Flutter ini, harap ikuti panduan berikut:
1. **Existing project structure is the reference**: Ikuti layout folder terbaru sebagai referensi utama.
2. **Consistency over perfection**: Ikuti naming convention dan coding style yang sudah ada.
3. **Minimal changes**: Buat perubahan seminimal mungkin, jangan lakukan refactor atau rewrite total kecuali benar-benar diperlukan.
4. **Design System**: Semua styling wajib mengikuti design tokens yang telah didefinisikan di `lib/theme/app_theme.dart`.
